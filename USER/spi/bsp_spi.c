...
static void Wait_For_Ready(void)
{
  ...
  } while ((FLASH_Status & 0x01) == SET); //返回 1 则忙，等待
...
{
  uint8_t Num_Page = 0, Num_Single = 0, Addr_Rem = 0, Align_Left = 0, Align_Right = 0;

  Num_Page = Num / SPI_FLASH_PageSize;        //可以整页写入多少次
  Num_Single = Num % SPI_FLASH_PageSize;      //整页写入后剩下多少个
  Addr_Rem = Addr % SPI_FLASH_PageSize;       //0 ：地址对齐，非0：没对齐
  Align_Left = SPI_FLASH_PageSize - Addr_Rem; //数据结束位置距离下一个对齐地址的个数
